...
namespace FC_EventDriven_PDP
{
    ...
    internal class Session
    {
        ...
        private async Task Start()
        {
            ...
            await OnBytesReceived();
        }

        private async Task OnBytesReceived()
        {
            _size = await Receive(_conn, _buffer, 0, _buffer.Length);
            ProcessBuffer();
        }

        private async Task SendHttpResponse(ResponseType responseType)
        {
        ...
        private async void ProcessBuffer()
        {
            if (_size == 0)
            {
                await SendHttpResponse(ResponseType.OK);
            }
            _pos = 0;
            while (_pos < _size)
            {
                ...
                if (_lastReadCharacter != null && _lastReadCharacter == '\r' && c == '\n')
                {
                    ...
                    if (_headerEndCount >= 2)
                    {
                        if (_headers.ContainsKey("Content-Length"))
                        {
                            ...
                            await future2.ContinueWith(OnReadBody);
                            return;
                        }
                        else
                        {
        ...
        private async Task OnReadBody(Task<int> f)
        {
            ...
            Task<int> future = Receive(_conn, _buffer, 0, _buffer.Length);
            await OnReadBody(future);
        }

        private Task OnSendDone(Task<int> future)
        {
            _conn.Close();
            Console.WriteLine("\nConnection closing ...");
